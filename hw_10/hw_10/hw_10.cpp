...
private:
    ...
    ID3D11VertexShader* m_pPostVS;
    ID3D11PixelShader* m_pPostPS;
...
DXApp::DXApp(HINSTANCE hInstance) :
    ...
    ZeroMemory(m_queries, sizeof(m_queries));
...
DXApp::~DXApp() {
    ...
    SAFE_RELEASE(m_pPostPS);
    SAFE_RELEASE(m_pPostVS);
...
bool DXApp::CreatePostProcessRT() {
    SAFE_RELEASE(m_pColorBufferSRV);
    SAFE_RELEASE(m_pColorBufferRTV);
    SAFE_RELEASE(m_pColorBuffer);
...
bool DXApp::Init() {
    ...
    // Буфер видимых индексов (приёмник для CopyResource)
    {
        D3D11_BUFFER_DESC desc = {};
        desc.ByteWidth = sizeof(Point4i) * MaxInstances;
        desc.Usage = D3D11_USAGE_DEFAULT;    
        desc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
        desc.CPUAccessFlags = 0;                        
    ...
    InstDesc descs4[4] = {
        { -1.1f, 0.0f,  0.0f,  0.8f,  48.0f, 0.0f }, // старая текстура
    ...
    if (!CreatePostProcessRT()) {
        MessageBox(nullptr, L"CreatePostProcessRT failed", L"Error", MB_OK);
        return false;
    }

    ID3DBlob* pPostVSCode = nullptr;
    result = CompileShaderFromFile(L"post.vs", &pPostVSCode);
    if (FAILED(result)) {
        MessageBox(nullptr, L"CompileShaderFromFile post.vs failed", L"Error", MB_OK);
        return false;
    }
    result = m_pDevice->CreateVertexShader(
        pPostVSCode->GetBufferPointer(), pPostVSCode->GetBufferSize(),
        nullptr, &m_pPostVS);
    SAFE_RELEASE(pPostVSCode);
    if (FAILED(result)) {
        MessageBox(nullptr, L"CreateVertexShader post failed", L"Error", MB_OK);
        return false;
    }

    ID3DBlob* pPostPSCode = nullptr;
    result = CompileShaderFromFile(L"post.ps", &pPostPSCode);
    if (FAILED(result)) {
        MessageBox(nullptr, L"CompileShaderFromFile post.ps failed", L"Error", MB_OK);
        return false;
    }
    result = m_pDevice->CreatePixelShader(
        pPostPSCode->GetBufferPointer(), pPostPSCode->GetBufferSize(),
        nullptr, &m_pPostPS);
    SAFE_RELEASE(pPostPSCode);
    if (FAILED(result)) {
        MessageBox(nullptr, L"CreatePixelShader post failed", L"Error", MB_OK);
        return false;
    }
    ...
    // ---- Compute shader (GPU frustum culling) ----
    {
        ...
        result = CompileShaderFromFile(L"cull.cs", &pCSCode);
        if (FAILED(result)) {
            MessageBox(nullptr, L"CompileShaderFromFile cull.cs failed", L"Error", MB_OK);
            return false;
        }
        result = m_pDevice->CreateComputeShader(
            pCSCode->GetBufferPointer(), pCSCode->GetBufferSize(),
            nullptr, &m_pCullShader);
        SAFE_RELEASE(pCSCode);
        if (FAILED(result)) {
            MessageBox(nullptr, L"CreateComputeShader failed", L"Error", MB_OK);
            return false;
        }
    }

    // CullParams constant buffer
    {
        D3D11_BUFFER_DESC desc = {};
        ...
        if (FAILED(result)) {
            MessageBox(nullptr, L"CreateBuffer CullParams failed", L"Error", MB_OK);
            return false;
        }
    }

    // IndirectArgs source buffer (UAV, structured, stride=sizeof(UINT))
    {
        D3D11_BUFFER_DESC desc = {};
        desc.ByteWidth = sizeof(UINT) * 5; // D3D11_DRAW_INDEXED_INSTANCED_INDIRECT_ARGS = 5 UINTs
        desc.Usage = D3D11_USAGE_DEFAULT;
        ...
        if (FAILED(result)) {
            MessageBox(nullptr, L"CreateBuffer IndirectArgsSrc failed", L"Error", MB_OK);
            return false;
        }
        result = m_pDevice->CreateUnorderedAccessView(m_pIndirectArgsSrc, nullptr, &m_pIndirectArgsUAV);
        if (FAILED(result)) {
            MessageBox(nullptr, L"CreateUAV IndirectArgs failed", L"Error", MB_OK);
            return false;
        }
    }

    // IndirectArgs draw buffer (MiscFlags = DRAWINDIRECT_ARGS)
    {
        D3D11_BUFFER_DESC desc = {};
        ...
        if (FAILED(result)) {
            MessageBox(nullptr, L"CreateBuffer IndirectArgs failed", L"Error", MB_OK);
            return false;
        }
    }

    // GeomBufferInstVisGPU (UAV, structured, для записи видимых id)
    {
        D3D11_BUFFER_DESC desc = {};
        desc.ByteWidth = sizeof(Point4i) * MaxInstances;
        desc.Usage = D3D11_USAGE_DEFAULT;
        ...
        if (FAILED(result)) {
            MessageBox(nullptr, L"CreateBuffer GeomBufferInstVisGPU failed", L"Error", MB_OK);
            return false;
        }
        result = m_pDevice->CreateUnorderedAccessView(m_pGeomBufferInstVisGPU, nullptr, &m_pGeomBufferInstVisGPU_UAV);
        if (FAILED(result)) {
            MessageBox(nullptr, L"CreateUAV GeomBufferInstVisGPU failed", L"Error", MB_OK);
            return false;
        }
    }
    {
        ...
        for (int i = 0; i < 10 && SUCCEEDED(result); i++) {
            ...
            if (FAILED(result)) {
                MessageBox(nullptr, L"CreateQuery failed", L"Error", MB_OK);
                return false;
            }
        }
    }
...
void DXApp::Render() {
    ...
    rawPlanes[0] = XMVectorAdd(vpT.r[3], vpT.r[2]);
    rawPlanes[1] = XMVectorSubtract(vpT.r[3], vpT.r[2]);
    rawPlanes[2] = XMVectorAdd(vpT.r[3], vpT.r[0]);
    rawPlanes[3] = XMVectorSubtract(vpT.r[3], vpT.r[0]);
    rawPlanes[4] = XMVectorSubtract(vpT.r[3], vpT.r[1]);
    rawPlanes[5] = XMVectorAdd(vpT.r[3], vpT.r[1]);
    for (int i = 0; i < 6; i++) {
        float len = XMVectorGetX(XMVector3Length(rawPlanes[i]));
        if (len > 0.0f) rawPlanes[i] = XMVectorScale(rawPlanes[i], 1.0f / len);
        sceneBuffer.frustum[i] = StorePoint4f(rawPlanes[i]);
    }
    ...
    for (UINT i = 0; i < (UINT)m_instances.size(); i++) {
        ...
        // AABB для compute shader (единичный куб, центр в (x,y,z))
        cullParams.bbMin[i] = Point4f(x - 0.5f, y - 0.5f, z - 0.5f, 1.0f);
        cullParams.bbMax[i] = Point4f(x + 0.5f, y + 0.5f, z + 0.5f, 1.0f);
    }
    ...
    // Обновить CullParams
    {
        ...
        m_pDeviceContext->Map(m_pCullParams, 0, D3D11_MAP_WRITE_DISCARD, 0, &ms);
        memcpy(ms.pData, &cullParams, sizeof(cullParams));
        m_pDeviceContext->Unmap(m_pCullParams, 0);
    }

    // Сбросить IndirectArgs: IndexCountPerInstance=36, InstanceCount=0, остальное=0
    {
        struct DrawArgs { UINT idx, inst, startIdx; INT baseVtx; UINT startInst; };
        DrawArgs args = { 36, 0, 0, 0, 0 };
        m_pDeviceContext->UpdateSubresource(m_pIndirectArgsSrc, 0, nullptr, &args, 0, 0);
    }

    // Запустить compute shader для GPU culling
    {
        UINT groupNumber = ((UINT)m_instances.size() + 63u) / 64u;
        ID3D11Buffer* csCBs[] = { m_pSceneBuffer, m_pCullParams };
        m_pDeviceContext->CSSetConstantBuffers(0, 2, csCBs);
        ID3D11UnorderedAccessView* csUAVs[] = { m_pIndirectArgsUAV, m_pGeomBufferInstVisGPU_UAV };
    ...
    // Draw instanced cubes с GPU culling
    {
        ...
        // Используем DrawIndexedInstancedIndirect — параметры с GPU
    ...
    wchar_t title[256];
    swprintf_s(title, L"HW1- | yaw: %.2f  pitch: %.2f | GPU visible instances: %d", m_yaw, m_pitch, m_gpuVisibleInstances);
    SetWindowText(m_hWnd, title);
    ReadQueries();
    D3D11_VIEWPORT postViewport = {};
    postViewport.Width = (FLOAT)m_width;
    postViewport.Height = (FLOAT)m_height;
    postViewport.MinDepth = 0.0f;
    postViewport.MaxDepth = 1.0f;
    m_pDeviceContext->RSSetViewports(1, &postViewport);

    //postprocessing
    {
        ID3D11RenderTargetView* bbViews[] = { m_pBackBufferRTV };
        m_pDeviceContext->OMSetRenderTargets(1, bbViews, nullptr);

        ID3D11ShaderResourceView* srvs[] = { m_pColorBufferSRV };
        ...
        m_pDeviceContext->Draw(3, 0); // один треугольник покрывает весь экран
    }
