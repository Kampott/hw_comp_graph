...
void DXApp::Render() {
    ...
    sceneBuffer.lightCount = Point4i(3, 0, 0, 0);
    ...
    {
        RenderItem item;
        item.position = Point3f(0.0f, 0.0f, 0.0f);
        item.scale = 1.0f;
        item.rotationSpeed = 1.0f;
        item.color = Point4f(1.0f, 1.0f, 1.0f, 1.0f);
        item.shininess = 48.0f;
        item.useNormalMap = true;
        opaqueItems.push_back(item);
    }
    {
        RenderItem item;
        item.position = Point3f(2.0f, 0.15f, 0.4f);
        item.scale = 0.75f;
        item.rotationSpeed = -0.7f;
        item.color = Point4f(1.0f, 1.0f, 1.0f, 1.0f);
        item.shininess = 24.0f;
        item.useNormalMap = true;
        opaqueItems.push_back(item);
    }
    ...
    for (const auto& item : opaqueItems) {
        ...
        RenderCube(model, item.color, item.shininess, item.useNormalMap, false);
    }
    ...
    {
        RenderItem item;
        item.position = Point3f(-1.35f, -0.1f, 1.2f);
        item.scale = 0.9f;
        item.rotationSpeed = 0.8f;
        item.color = Point4f(0.3f, 0.8f, 1.0f, 0.35f);
        item.shininess = 16.0f;
        item.useNormalMap = true;
        transparentItems.push_back(item);
    }
    {
        RenderItem item;
        item.position = Point3f(1.25f, 0.35f, -1.35f);
        item.scale = 0.85f;
        item.rotationSpeed = -1.1f;
        item.color = Point4f(1.0f, 0.45f, 0.35f, 0.35f);
        item.shininess = 20.0f;
        item.useNormalMap = true;
        transparentItems.push_back(item);
    }
    ...
    for (const auto& item : transparentItems) {
        ...
        RenderCube(model, item.color, item.shininess, item.useNormalMap, true);
    }
