...
struct GeomBuffer {
    XMFLOAT4X4 model;
    XMFLOAT4   color;
    XMFLOAT4   params;   // params.x = scale for skybox, otherwise unused
};
...
private:
    ...
    void RenderCube(const XMMATRIX& model, const XMFLOAT4& color, bool transparent);
    void RenderSkybox();
...
bool DXApp::LoadTexture(const std::wstring& TextureName) {
    ...
    if (!LoadDDS(TextureName.c_str(), textureDesc, false)) {
        return false;
    }
    ...
    HRESULT result = m_pDevice->CreateTexture2D(&desc, subData.data(), &m_pTexture);
...
bool DXApp::Init() {
    ...
    m_hWnd = CreateWindow(
        L"DXAppClass", L"Computer Graphics HW5",
        WS_OVERLAPPEDWINDOW,
...
void DXApp::RenderCube(const XMMATRIX& model, const XMFLOAT4& color, bool transparent) {
    GeomBuffer geomBuffer = {};
    XMStoreFloat4x4(&geomBuffer.model, model);
    geomBuffer.color = color;
    geomBuffer.params = XMFLOAT4(1.0f, 0.0f, 0.0f, 0.0f);
...
void DXApp::RenderSkybox() {
    GeomBuffer geomBuffer = {};
    XMStoreFloat4x4(&geomBuffer.model, XMMatrixIdentity());
    geomBuffer.color = XMFLOAT4(1.f, 1.f, 1.f, 1.f);
    geomBuffer.params = XMFLOAT4(25.0f, 0.0f, 0.0f, 0.0f);
...
void DXApp::Render() {
    ...
    // Reversed depth: near/far swapped.
    ...
    // Build opaque objects.
    std::vector<RenderItem> opaqueItems;
    {
        RenderItem item;
        item.position = XMFLOAT3(0.0f, 0.0f, 0.0f);
        item.scale = 1.0f;
        item.rotationSpeed = 1.0f;
        item.color = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
        opaqueItems.push_back(item);
    }
    {
        RenderItem item;
        item.position = XMFLOAT3(2.0f, 0.15f, 0.4f);
        item.scale = 0.75f;
        item.rotationSpeed = -0.7f;
        item.color = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
        opaqueItems.push_back(item);
    }

    for (auto& item : opaqueItems) {
        XMVECTOR pos = XMVectorSet(item.position.x, item.position.y, item.position.z, 1.0f);
        item.sortKey = XMVectorGetX(XMVector3LengthSq(XMVectorSubtract(pos, eye)));
    }
    ...
    for (const auto& item : opaqueItems) {
        XMMATRIX model =
            ...
            XMMatrixRotationY(item.rotationSpeed * m_time) *
            XMMatrixTranslation(item.position.x, item.position.y, item.position.z);

        RenderCube(model, item.color, false);
    }

    // Skybox after opaque, before transparent.
    RenderSkybox();

    // Build transparent objects and sort far -> near.
    std::vector<RenderItem> transparentItems;
    {
        RenderItem item;
        item.position = XMFLOAT3(-1.35f, -0.1f, 1.2f);
        item.scale = 0.9f;
        item.rotationSpeed = 0.8f;
        item.color = XMFLOAT4(0.3f, 0.8f, 1.0f, 0.45f);
        transparentItems.push_back(item);
    }
    {
        RenderItem item;
        item.position = XMFLOAT3(1.25f, 0.35f, -1.35f);
        item.scale = 0.85f;
        item.rotationSpeed = -1.1f;
        item.color = XMFLOAT4(1.0f, 0.45f, 0.35f, 0.45f);
        transparentItems.push_back(item);
    }
    ...
    for (const auto& item : transparentItems) {
        ...
        RenderCube(model, item.color, true);
    }

    wchar_t title[256];
    swprintf_s(title, L"HW5 | yaw: %.2f  pitch: %.2f  radius: %.2f", m_yaw, m_pitch, m_radius);
